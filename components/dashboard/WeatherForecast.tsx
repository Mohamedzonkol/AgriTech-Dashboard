...
const WeatherForecast = ({
  ...
  const getWeatherIcon = (condition: string) => {
    ...
    switch (condition.toLowerCase()) {
      ...
      case "snow":
        return <CloudSnow className={`${iconClasses} text-blue-300`} />;
      case "partly-cloudy":
  ...
  if (loading) {
    return (
      <div
        className={`bg-white/30 dark:bg-gray-800 rounded-xl shadow-lg p-6 ${className}`}
      >
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center py-8 text-gray-600">
          <Cloud className="w-10 h-10 mb-2 text-gray-400" />
  ...
  return (
    <div
      className={`backdrop-blur-md bg-white/30 dark:bg-gray-900 border border-gray-200 rounded-xl shadow-lg p-6 flex flex-col col-span-1 transition-transform hover:scale-105 duration-300 ${className}`}
    >
      ...
      <div className="space-y-4">
        {forecast.map((day) => (
          <div
            key={day.date}
            className="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-gray-800 hover:shadow-md transition-shadow transition-transform hover:scale-105 duration-300"
          >
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="font-medium w-8 text-gray-700 dark:text-gray-200">
                {Math.round(day.high)}°
              </span>
            </div>
